...
class GeometricFeatureExtractor:
    ...
    def wavelet_features(self, spec: np.ndarray) -> list[float]:
        ...
        # Wavelet szint meghatározása (integer!)
        max_level = int(np.floor(np.log2(N)))
        level = max(1, min(max_level, 5))  # stabil, nem engedjük túl nagyra
        ...
        for c in coeffs:
            ...
            # F57–F81: minden szinten 5 jellemző
            # (energia, átlag, szórás, abszolút átlag, entropia)
            energy = float(np.sum(c**2))
            mean = float(np.mean(c))
            std = float(np.std(c))
            abs_mean = float(np.mean(np.abs(c)))

            P = np.abs(c) + eps
            P = P / np.sum(P)
            entropy = float(stats.entropy(P))

            features.extend([energy, mean, std, abs_mean, entropy])

        # Ha kevesebb mint 25 érték jött ki (ritka), kipótoljuk nullákkal
        while len(features) < 25:
            features.append(0.0)

        # Csak az első 25 értéket tartjuk meg (F57–F81)
        return features[:25]
    def texture_features(self, spec: np.ndarray) -> list[float]:
        """
        Kategória VI – Textúra jellemzők (F82–F92)
        Stabil, GLCM-alapú textúra jellemzők.
        """
        ...
        # 2) GLCM számítás
        # Csak 1 irányt használunk (0°), 1 pixeles távolsággal
        try:
            glcm = greycomatrix(
                img_norm,
                distances=[1],
            ...
            # Ha valamiért nem sikerül → 11 db 0
            return [0.0] * 11
        ...
        # 4) Saját textúra jellemzők
        entropy = float(stats.entropy((glcm + eps).flatten()))
